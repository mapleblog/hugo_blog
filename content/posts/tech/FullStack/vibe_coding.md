...
- 你可以给 AI 发这样一句话：
  ...
  ~~- Next.js, Prisma, Tailwind。为了让你更好的写出vibe coding提示词，你可以向我提问挖掘你所有需要的**项目细节**，**项目核心需求**，**画出/列出数据结构**和**定义视觉风格**，同时也需要麻烦你考量这个项目是否需要：
  ~~- 需要判断受否需要后期扩展声明，方便后期新功能开发
  ~~- 需要声明代码规范性规则，确保代码结构一致性
  ~~- 采用适合对项目的安全保护机制
  ~~直到你认为可以开始写提示词为止。然后在当前路径以**英文版本**生成完整详细的第一步Phase_1_PRD.md核心内容。
  
  =====
  
  -----
  根据刚才你提供的PRD文档,麻烦你进行第二步"项目基础架构配置安装与部署", 同样使用**结构化上下文框架 (Structured Context Framework)** 结合 **Role-Context-Constraints (RCC)** 写生成**英文**完整和详细的提示词，谢谢你！✅
